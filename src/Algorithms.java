...
/**
 * sorting and oprations to handle 'DIDTINCT' in this part, also contains disk I/O functions
 */
...
//        ArrayList<Tuple> tuplesToReturn = new ArrayList<Tuple>();
//        tuplesToReturn.add(tuples.get(0));
//        Tuple tuple = tuples.get(0);
//        if (fieldNames.get(0).equals("*")) {
//            fieldNames = tuple.getSchema().getFieldNames();
//        }
...
//        while (pointer2 < tuples.size()) {
//            int temp = 1;
//            for(int i = 0; i <= tuplesToReturn.size(); i++) {
//                if (compareTuple(tuplesToReturn.get(i), tuples.get(pointer2), fieldNames) != 0) {
//                    temp = 0;
//                    break;
//                }
//            }
//            if(temp == 1) {
//                System.out.print(tuplesToReturn.size());
//                System.out.print("\n");
//                tuplesToReturn.add(tuples.get(pointer2));
//                pointer1++;
//            }
//        }
//        pointer2 += 1;
...
//            tuples.remove(i);
//        }
//        tuples = tuplesToReturn;
    }

    public static int compareTuple(Tuple o1, Tuple o2, List<String> fieldNames) {
        for (String name: fieldNames){
            if (o1.getField(name).type == FieldType.INT) {
                int comInt = Integer.compare(o1.getField(name).integer, o2.getField(name).integer);
                if (comInt!= 0) {
                    return comInt;
                }
            } else {
                int comSTR = o1.getField(name).str.compareTo(o2.getField(name).str);
                if (comSTR != 0) {
                    return comSTR;
                }
            }
    ...
    public static int compareTuple(Tuple o1, Tuple o2, List<String> fieldNames, String orderBy) {
        if(o1.getField(orderBy).type == FieldType.INT) {
            int com_int = Integer.compare(o1.getField(orderBy).integer, o2.getField(orderBy).integer);
            if(com_int!=0)
                return com_int;
        } else {
            int com_str = o1.getField(orderBy).str.compareTo(o2.getField(orderBy).str);
            if(com_str !=0)
                return com_str;
        }
        for (String name: fieldNames){
    ...
    //Two pass sort and read blocks into memory.
    ...
    public static HashSet<String> tableInExpression(Statement statement) {
        statement.contains_table = new HashSet<>();
        if(statement.getContent().equalsIgnoreCase("FieldName")) {
            assert statement.getSubstatement().size() > 1;
            statement.contains_table.add(statement.getSubstatement().get(0).getContent());
            return statement.contains_table;
        } else {
            if(statement.getSubstatement()==null||statement.getSubstatement().size()==0) {
                return statement.contains_table;
            }else {
                for (Statement subStatement: statement.getSubstatement()) {
                    statement.contains_table.addAll(tableInExpression(subStatement));
                }
                return statement.contains_table;
            }
        }
...
//    merge into a statement
    public static void mergeField(Statement statement) {

        if (statement.getContent().equalsIgnoreCase("FieldName")) {
            List<Statement> cols = statement.getSubstatement();
            StringBuilder s = new StringBuilder();
            List<Statement> tempBranches = new ArrayList<>();
            for (Statement field: cols) {
                s.append(field.getContent()+".");
            }
            s.deleteCharAt(s.length()-1);
            tempBranches.add(new Statement(s.toString(), true));
            statement.setSubstatement(tempBranches);
        } else {
            if(statement.getSubstatement()!=null&&statement.getSubstatement().size()>0) {
                for (Statement st : statement.getSubstatement()) {
                    mergeField(st);
                }
            }
        }
